...
import capstoneCodeSnippet from '../images/capstone_code.png';
...
import tuxedoCatCodeSnippet from '../images/tuxedoCat_code.png';
...
export const projects = {
    "capstone": {
        ...
        briefDescription: "Wedding photography site built in WordPress",
        ...
        highlight: "We wanted to showcase the photography on a portfolio page, with two different ways of laying out the imagery: you could view the photos by project (or \"stories\"), which would show the project's featured image and title with a link to the project, or you could view by category, which would showcase a grid of all photos belonging to that category. These layouts would both utilize the isotope plugin, and the category view would also use the lightgallery plugin.\nImplementing these features required a number of conditional statements to ensure the correct formatting was used depending on the user's choice, which is selected by a series of links at the top of the page. The plugins are loaded on to the required page in functions.php, and then applied to the images via classes, with some custom styling applied depending on which condition is met.",
        highlightImage: capstoneCodeSnippet,
    ...
    "tuxedocat": {
        ...
        highlight: "In the design phase, I decided I wanted to try to implement a responsive design on the \"Meet the cats\" page, which would change styles based on the screen size. On mobile, it would appear as a series of boxes, linked by vertical lines, with a photo of the cat and a small description inside. On desktop, those boxes would change into a snaking pattern, and the cat info would alternate being on the right or left side, following along with the snake.\nThrough what I think was some clever use of classes and CSS, I managed to do this all with simple divs, which would lose one side of their outline depending on their order, and overlap slightly. It took some trial and error to get the placement just right for everything, but I think the end result came out pretty neat!",
        highlightImage: tuxedoCatCodeSnippet,
    ...
    /*"boxclicker": {
        ...
        tools: ["Google"],
        description: "Box Clicker was my first attempt at working with JavaScript. After the first week of class, I wanted to see if I could take the fundamentals that I had already learned, and apply them to an idea just outside of my knowledge range. I came up with the idea for a simple website, where you could click a button to infinitely subdivide into smaller boxes.",
